...
trait AdminAuthTrait
{
    ...
    protected function checkLogin(): bool
    {
        ...
        // 获取当前访问url,应用名+'/'+控制器名
        $controller_url = parse_name(app('http')->getName())
            . '/' . parse_name($request->controller());

        $login_except = !empty($this->loginExcept) ? array_map('parse_name', $this->loginExcept) : $this->loginExcept;
        $no_login_controller = !empty($this->no_login_controller) ? array_map('parse_name', $this->no_login_controller) : $this->no_login_controller;

        if (in_array($url, $login_except, true) || in_array($controller_url, $no_login_controller, true)) {
            return true;
        }
